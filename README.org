...
**** Solution
     ...
     Note that this problem is an example of the knapsack problem. A naive
     solution would reuse the knapsack problem's canonical DP-based solution to
     solve this problem in O(n^2). This approach is naive due to its attempt to
     reduce a simpler problem (this one) into a more difficult one (the knapsack
     problem). Specifically, we note that the problem doesn't ask for exactly
